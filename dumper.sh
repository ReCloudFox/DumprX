...
# Password-Protected Archive Check
if [[ -n "${DUMPRX_ARCHIVE_PASSWORD}" && -f "${FILEPATH}" ]]; then
	if ! "${BIN_7ZZ}" t -y "-p" "${FILEPATH}" >/dev/null 2>&1 && "${BIN_7ZZ}" t -y "-p${DUMPRX_ARCHIVE_PASSWORD}" "${FILEPATH}" >/dev/null 2>&1; then
		printf "Password-Protected Archive Detected. Decrypting...\n"
		mkdir -p "${INPUTDIR}"/decrypted
		"${BIN_7ZZ}" x -y "-p${DUMPRX_ARCHIVE_PASSWORD}" "${FILEPATH}" -o"${INPUTDIR}"/decrypted >/dev/null
		( cd "${INPUTDIR}"/decrypted && "${BIN_7ZZ}" a -tzip "${INPUTDIR}/${FILE%.*}_plain.zip" . >/dev/null )
		rm -rf "${INPUTDIR:?}"/decrypted
		rm -f "${FILEPATH}"	# Remove The Original Encrypted Archive So The Reload Sees A Single Input File
		unset FILEPATH
		cd "${PROJECT_DIR}" || exit
		( bash "${0}" "${INPUTDIR}/${FILE%.*}_plain.zip" ) || exit 1
		exit
	fi
fi
...
if ${BIN_7ZZ} l -ba "${FILEPATH}" | grep -q "system.new.dat" 2>/dev/null || [[ $(find "${TMPDIR}" -type f -name "system.new.dat*" -print | wc -l) -ge 1 ]]; then
	...
	${BIN_7ZZ} e -y -r "${FILEPATH}" '*super.img' dummypartition 2>/dev/null >> ${TMPDIR}/zip.log
	if [[ ! -f super.img ]]; then
